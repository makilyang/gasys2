class MainMenu extends BaseScene {
	...
	public initialize() {
		if (!this._initialized) {
			this._view = fairygui.UIPackage.createObject("tduilite", "001 封面").asCom;
			fairygui.GRoot.inst.addChild(this._view);
			// this._view.setSize(fairygui.GRoot.inst.width, fairygui.GRoot.inst.height);
			// this._view.setSize(G.width * G.scale, G.height * G.scale);
			this._view.setSize(G.width , G.height);
			super.initialize();
			var btn: fairygui.GButton = this._view.getChild("btnstart").asButton;
			btn.addClickListener(this.onStart, this);
			var btn1: fairygui.GButton = this._view.getChild("btninfo").asButton;
	...
	private onTuorial(e: egret.Event): void {
		let comp = fairygui.UIPackage.createObject("tduilite", "005 玩法说明").asCom;
