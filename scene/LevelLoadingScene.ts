class LevelLoadingScene extends BaseScene {
    ...
    private async loadResource(args: string[]) {
        try {
            ...
            RES.createGroup("temp", args, true);
            ...
            if (this._mapConfig.id == "m001") {
                ...
                comp.getChild("n1").asLoader.url = this.getResPath("c001_png");
                let i: number = 1;
                let func = (e: egret.TouchEvent) => {
                    let ctrl = comp.getController("c1");
                    if (i == 1)
                        comp.getChild("n2").asLoader.url = this.getResPath("c002_png");
                    if (i == 2)
                        comp.getChild("n3").asLoader.url = this.getResPath("c003_png");;

                    if (i == 3) {
                        comp.getChild("n2").asLoader.url = "";
                        comp.getChild("n3").asLoader.url = "";
                        comp.getChild("n1").asLoader.url = this.getResPath("c004_png");;
                        ctrl.selectedIndex = 1;
                    }
                    if (i == 4)
                        comp.getChild("n2").asLoader.url = this.getResPath("c005_png");;
                    if (i == 5)
                        comp.getChild("n3").asLoader.url = this.getResPath("c006_png");;
                    if (i == 6) {
                        comp.getChild("n4").asLoader.url = this.getResPath("c007_png");;
    ...
    private getResPath(key: string) {
        let rescfg = RES.getResourceInfo(key);
        return rescfg.root + rescfg.url;
