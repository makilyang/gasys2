class GameScene extends BaseScene implements INotifier {
	...
	private _babbtn: fairygui.GButton;
	public constructor() {
		super();
	...
	public initialize() {
		if (!this._initialized) {
			...
			this._babbtn = this._view.getChild("n31").asButton;
			this._babbtn.addEventListener(egret.TouchEvent.TOUCH_END, this.onBag, this);
		...
		RES.createGroup("temp", [ "ta1_json","ta1_png",], true);
		RES.loadGroup("temp");
	}
	...
	private onBag(e: egret.TouchEvent) {

		// this._babbtn.selected = !this._babbtn.selected;
		egret.log("bag:", this._babbtn.selected);
		let bar = this._view.getChild("n32") as gui.PropsBar
		if (!this._babbtn.selected)
			bar.show();
		else
			bar.show(false);
